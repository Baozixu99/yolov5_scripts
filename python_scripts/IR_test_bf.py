...
def Inference(compiled_model, image_file):
    ...
    input_shape = input_tensor.shape  # 应为 [1,256,256,3]
    h, w = input_shape[1], input_shape[2]  # 高度和宽度

    # 预处理流程（生成 NHWC 格式）
    img = cv2.resize(img0, (w, h))          # 调整尺寸至 (256,256)
    img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)  # 转换为 RGB，形状 (256,256,3)
    img = np.expand_dims(img, axis=0)       # 添加批次维度，形状 (1,256,256,3)
    ...
    return torch.from_numpy(output_tensor.data), inference_time
...
def start_inference(compiled_model, img_dir):
    # IMAGE_DIR = '/home/bousew/jupyterWorkspace/is2ros_yolov5_inference/labels_and_dataset/dataset/fire_smoke/train/images'
    IMAGE_DIR = img_dir
    # LABELS_DIR = '/home/bousew/jupyterWorkspace/is2ros_yolov5_inference/labels_and_dataset/dataset/fire_smoke/train/labels'
    # 初始化统计量
    total_images = 0
    total_correct = 0
    #total_predictions = 0
    ...
    prediction, elapsed_time = Inference(compiled_model, IMAGE_DIR)
...
def inference_worker(q, shutdown_event, compiled_model):
    while not shutdown_event.is_set() or not q.empty():
        try:
            ...
            # IMAGE_DIR = 'H:\\jupyterWorkspace_BAC\\is2ros_yolov5_inference\\labels_and_dataset\\dataset\\my_data\\' + img_name
            IMAGE_DIR = '/home/jupyterWorkspace/is2ros_yolov5_inference/labels_and_dataset/dataset/my_data/'+ img_name
            #IMAGE_DIR ='H:\jupyterWorkspace_BAC\is2ros_yolov5_inference\yolov5_scripts\python_scripts\80uav3.png'
            # IMAGE_DIR = './'
            #print("IMAGE_DIR is: ")
            print(IMAGE_DIR)
...
if __name__ == "__main__":
    ...
    # uav_configs = [
    #     (8891, "H:\\jupyterWorkspace_BAC\\is2ros_yolov5_inference\\labels_and_dataset\\dataset\\my_data\\", "uav1"),
    #     (8892, "H:\\jupyterWorkspace_BAC\\is2ros_yolov5_inference\\labels_and_dataset\\dataset\\my_data\\", "uav2"),
    #     (8893, "H:\\jupyterWorkspace_BAC\\is2ros_yolov5_inference\\labels_and_dataset\\dataset\\my_data\\", "uav3"),
    # ]
    uav_configs = [
        (8891, "/home/jupyterWorkspace/is2ros_yolov5_inference/labels_and_dataset/dataset/my_data/", "uav1"),
        (8892, "/home/jupyterWorkspace/is2ros_yolov5_inference/labels_and_dataset/dataset/my_data/", "uav2"),
        (8893, "/home/jupyterWorkspace/is2ros_yolov5_inference/labels_and_dataset/dataset/my_data/", "uav3"),
    ]
    q = queue.Queue()
    threads = []
    ...
    # 模型路径
    # MODEL_XML = 'H:\jupyterWorkspace_BAC\is2ros_yolov5_inference\yolov5_scripts\IR_models\IR_model_with_ovc\\best.xml'
    # MODEL_BIN = 'H:\jupyterWorkspace_BAC\is2ros_yolov5_inference\yolov5_scripts\IR_models\IR_model_with_ovc\\best.bin'
